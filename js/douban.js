...
function renderDoubanCards(data, container) {
    ...
    if (!data.subjects || data.subjects.length === 0) {
        ...
        data.subjects.forEach(item => {
            ...
            // 主代理地址（直接拼接，不要encodeURIComponent）
            const proxiedCoverUrl = PROXY_URL + originalCoverUrl;
            // 备用代理地址（防止主代理失效）
            const fallbackProxyUrl = 'https://api.allorigins.win/raw?url=' + originalCoverUrl;
            ...
            card.innerHTML = `
                <div class="relative w-full aspect-[2/3] overflow-hidden cursor-pointer" onclick="fillAndSearchWithDouban('${safeTitle}')">
                    <img src="${proxiedCoverUrl}" alt="${safeTitle}" 
                            ...
                            onerror="this.onerror=null; this.src='${fallbackProxyUrl}'; this.classList.add('object-contain');"
